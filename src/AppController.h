 ...
 * @brief Top-level application coordinator managing dual panels and commander workflows.
 *
 * ARCHITECTURAL ROLE:
 * AppController is the root orchestrator of the commander application:
 *   - Owns and manages the two PanelController instances (leftPanel and rightPanel).
 *   - Tracks which panel is the source of user commands (activePanel) and which is
 *     the destination for dual-pane file operations (targetPanel).
 *   - Owns the centralized FileOperationsService (background worker) and FilePreviewService.
 *   - Translates high-level commander hotkeys and menu actions:
 *       * F5 (Copy): Reads selected items from activePanel, invokes FileOperationsService::copyItems to targetPanel path.
 *       * F6 (Move): Reads selected items from activePanel, invokes FileOperationsService::moveItems to targetPanel path.
 *       * F8 (Delete): Reads selected items from activePanel, invokes FileOperationsService::deleteItems.
 *       * Tab / Ctrl+U: Toggles or swaps active and target panels.
 *       * Session Save/Load: Persists panel directories across application runs.
 */
...
class AppController : public QObject
{
    ...
    Q_PROPERTY(int activePanelIndex READ activePanelIndex WRITE setActivePanelIndex NOTIFY activePanelIndexChanged)
    Q_PROPERTY(PanelController* activePanel READ activePanel NOTIFY activePanelIndexChanged)
    Q_PROPERTY(PanelController* targetPanel READ targetPanel NOTIFY activePanelIndexChanged)
    ...
    Q_PROPERTY(QStringList draggedPaths READ draggedPaths WRITE setDraggedPaths NOTIFY draggedPathsChanged)
...
public:
    ...
    PanelController* activePanel() const { return (m_activePanelIndex == 0) ? m_leftPanel : m_rightPanel; }
    PanelController* targetPanel() const { return (m_activePanelIndex == 0) ? m_rightPanel : m_leftPanel; }
     ...
     */
    Q_INVOKABLE void copySelected(const QStringList &customPaths = QStringList(), const QString &customDestination = QString());

    /**
     * @brief Move selected items from activePanel into targetPanel's directory.
     */
    Q_INVOKABLE void moveSelected(const QStringList &customPaths = QStringList(), const QString &customDestination = QString());

    /**
     * @brief Delete selected items from activePanel (Recycle Bin or permanent).
     */
    Q_INVOKABLE void deleteSelected(const QStringList &customPaths = QStringList(), bool permanent = false);

    /**
     * @brief Get selected paths from activePanel, or currently focused item if none selected.
     */
     ...
     */
    Q_INVOKABLE bool renameActiveItem(const QString &oldPath, const QString &newName);
    ...
    /**
     * @brief Open a system terminal (PowerShell / CMD) inside activePanel's current directory.
     */
